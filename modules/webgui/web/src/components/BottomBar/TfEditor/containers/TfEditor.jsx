...
class TfEditor extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (this.state.currentVolume === null && this.props.volumes.length !== 0) {
      ...
      const URI = this.props.volumes[0].data.properties.find(function (obj) { 
            return obj.id === "TransferFunction"; }).Description.Identifier;
      this.setState({URI});
    }
    if (this.state.color !== prevState.color) {
      const { currentVolume } = this.state;
      this.props.ChangeColor(this.state.color, this.props.volumes.find(function (obj) 
        { return obj.id === currentVolume.name; }).data.properties.find(function (obj) 
        { return obj.id === "TransferFunction"; }).Description.Identifier);
    }
  }
  ...
  handleVolumeChange(event) {
    ...
    const URI = this.props.volumes.find(obj => { 
      return obj.id === this.state.currentVolume.name; }).data.properties.find(function (obj) { 
        return obj.id === "TransferFunction"; }).Description.Identifier;
    this.setState({URI});
  }
                ...
                <div className={styles.Canvas}>
                  <div className={styles.FlexContainer}>
                    <EditorContainer
                      ...
                      activeVolume={volumes.find(function (obj) { return obj.id === currentVolume.name; }).data}
                      URI={URI}
                    />
                    <div className={styles.FlexColumn}>
                      <ColorPicker className={styles.ColorPicker} onColorChange={(color) => this.setState({color})} />
                      <div className={styles.FlexColumn}>
                        ...
                        <Select onChange={this.handleVolumeChange}
                          ...
                          options={this.props.volumes.map(volume => { return  { value: volume.name, label: volume.name}})}
                        />
                      </div>
                    </div>
                  </div>
