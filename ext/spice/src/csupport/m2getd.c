/* m2getd.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int m2getd_(char *name__, char *string, logical *found, 
	doublereal *dp, ftnlen name_len, ftnlen string_len)
{
    /* Builtin functions */
    ...
    extern /* Subroutine */ int m2vget_(char *, integer *, logical *, integer 
	    *, integer *, ftnlen), nparsd_(char *, doublereal *, char *, 
	    integer *, ftnlen, ftnlen), sigerr_(char *, ftnlen), chkout_(char 
	    *, ftnlen), setmsg_(char *, ftnlen);

/* $ Abstract */

/*     Select the Nth substring associated with a matched, named META/2 */
/*     template word and parse it as a double precision number. */
...
/*     DP         O   matching d.p. extracted and parsed from STRING. */
...
/*     DP         is the double precision number represented by the word */
/*                of STRING that was the first match with the NAMEd */
...
/*     2) If the portion of STRING extracted cannot be parsed as a */
/*        a double precision number, the error 'META/2(CORRUPTEDNUMBER)' */
/*         will be signalled. */
...
/*     Extract first number matching a named template word */
...
/*     Parse it. */
    ...
    nparsd_(string + (b - 1), &mydp, error, &pnter, e - (b - 1), (ftnlen)80);
    if (s_cmp(error, " ", (ftnlen)80, (ftnlen)1) != 0) {
	chkin_("M2GETD", (ftnlen)6);
	setmsg_("The item requested could not be parsed as an integer. a num"
		"ber.", (ftnlen)63);
	sigerr_("META/2(CORRUPTEDNUMBER)", (ftnlen)23);
	chkout_("M2GETD", (ftnlen)6);
	return 0;
    }
