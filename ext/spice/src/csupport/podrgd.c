/* podrgd.f -- translated by f2c (version 19980913).
...
/* $Procedure      PODRGD ( Pod, replace group, double precision ) */
/* Subroutine */ int podrgd_(doublereal *pod)
{
    ...
    extern /* Subroutine */ int chkin_(char *, ftnlen), dcoded_(doublereal *, 
	    integer *), scardd_(integer *, doublereal *);
    integer newoff;
    extern /* Subroutine */ int podond_(doublereal *, integer *, integer *);
...
/*     anything. */
    ...
    if (offset != 0) {
	pod[3] = pod[offset + 5];
	dcoded_(&pod[offset + 5], &newoff);
	i__1 = number;
	for (i__ = 1; i__ <= i__1; ++i__) {
	    pod[newoff + i__ + 5] = pod[offset + i__ + 5];
	}
	i__1 = newoff + number;
	scardd_(&i__1, pod);
    }
    chkout_("PODRGD", (ftnlen)6);
    return 0;
} /* podrgd_ */
