/* poddgd.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int poddgd_(doublereal *pod)
{
    integer need, have;
    extern integer cardd_(doublereal *);
    extern /* Subroutine */ int chkin_(char *, ftnlen);
    extern integer sized_(doublereal *);
    extern /* Subroutine */ int podaed_(doublereal *, integer *, doublereal *)
	    , podbgd_(doublereal *), podond_(doublereal *, integer *, integer 
	    *);
...
/*     elements, then one for future use.) */
    ...
    have = sized_(pod);
    need = cardd_(pod) + 1 + max(1,number);
    if (have < need) {
	sigerr_("SPICE(TOOMANYPEAS)", (ftnlen)18);
	chkout_("PODDGD", (ftnlen)6);
	return 0;
    }
...
/*     arent't changed by the addition of new ones. */
    ...
    podaed_(&pod[offset + 6], &number, pod);
    chkout_("PODDGD", (ftnlen)6);
    return 0;
} /* poddgd_ */
