/* m2getb.f -- translated by f2c (version 19980913).
...
/* $Procedure M2GETB ( META/2 --- get a named integer) */
/* Subroutine */ int m2getb_(char *name__, char *string, logical *found, 
	integer *int__, ftnlen name_len, ftnlen string_len)
{
    /* Builtin functions */
...
/*     Select the first substring associated with a matched, named META/2 */
/*     template word and put it parse it as an integer. */
...
/*     INT        O   matching integer extracted and parsed from STRING. */
...
/*                specified by NAME and STRING and NTH could be */
/*                retrieved. Otherwise it will be returned with a value */
/*                of .FALSE. */

/*     INT        is the ID-code represented by the word of STRING that */
/*                was the first matched with the NAMEd META/2 template */
/*                word. */
...
/*        body, the error 'META/2(CORRUPTEDBODYSPEC)' will be */
/*        signalled. */
...
/*        M2GETB  --- get and parse a body name */
...
/*            CALL M2GETB ( 'body1',     STRING, FOUND,  BODY1  ) */
/*            CALL M2GETB ( 'body2',     STRING, FOUND,  BODY2  ) */
/*            CALL M2GETB ( 'observer',  STRING, FOUND,  OBS    ) */
...
/*     Extract first integer matching a named template word */
...
/*     First see if we've got a recognized name. */
...
/*        Try an integer. */
	...
	if (s_cmp(error, " ", (ftnlen)80, (ftnlen)1) != 0) {
	    chkin_("M2GETB", (ftnlen)6);
	    setmsg_("The item requested, '#',  was not a recognized body and"
		    " could not be parsed as an integer. ", (ftnlen)91);
	    errch_("#", string + (b - 1), (ftnlen)1, e - (b - 1));
	    sigerr_("META/2(CORRUPTEDBODYNAME)", (ftnlen)25);
	    chkout_("M2GETB", (ftnlen)6);
	    return 0;
	}
    }
