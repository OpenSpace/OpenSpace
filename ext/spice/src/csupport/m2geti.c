/* m2geti.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int m2geti_(char *name__, char *string, logical *found, 
...
/*     INT        is the integer represented by the word of STRING that */
...
/*     Parse it. */
    ...
    nparsi_(string + (b - 1), &myint, error, &pnter, e - (b - 1), (ftnlen)80);
    if (s_cmp(error, " ", (ftnlen)80, (ftnlen)1) != 0) {
	chkin_("M2GETI", (ftnlen)6);
	setmsg_("The item requested could not be parsed as an integer.", (
		ftnlen)53);
	sigerr_("META/2(CORRUPTEDINTEGER)", (ftnlen)24);
	chkout_("M2GETI", (ftnlen)6);
	return 0;
    }
