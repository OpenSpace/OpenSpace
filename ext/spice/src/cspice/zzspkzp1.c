/* zzspkzp1.f -- translated by f2c (version 19980913).
	...
	ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int zzrefch1_(integer *, integer *, doublereal *, 
	    doublereal *), zzspkpa1_(integer *, doublereal *, char *, 
    ...
    extern /* Subroutine */ int zzspksb1_(integer *, doublereal *, char *, 
	    doublereal *, ftnlen), zzspkgp1_(integer *, doublereal *, char *, 
...
/*        call SPKSSB, SPKAPO and return. */
	...
	if (type__ == 1) {
	    ...
	    zzspkpa1_(targ, et, ref, sobs, abcorr, ptarg, lt, ref_len, 
		    abcorr_len);
	    chkout_("ZZSPKZP1", (ftnlen)8);
	    return 0;
	}
...
/*        We also need the light time to the center of the frame. */
	...
	zzspksb1_(obs, et, "J2000", sobs, (ftnlen)5);
	zzspkpa1_(targ, et, "J2000", sobs, abcorr, postn, lt, (ftnlen)5, 
		abcorr_len);
	if (failed_()) {
	    chkout_("ZZSPKZP1", (ftnlen)8);
	    return 0;
	...
	if (center == *obs) {
	    ...
	    zzspkpa1_(&center, et, "J2000", sobs, abcorr, temp, &ltcent, (
		    ftnlen)5, abcorr_len);
	}
...
/*        and convert the position. */
	...
	d__1 = *et - ltcent;
	zzrefch1_(&fj2000, &reqfrm, &d__1, xform);
	if (failed_()) {
	    chkout_("ZZSPKZP1", (ftnlen)8);
	    return 0;
	}
