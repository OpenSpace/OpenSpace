/* zzekllec.f -- translated by f2c (version 19980913).
...
/* $Procedure ZZEKLLEC ( EK, last less than or equal to, character ) */
/* Subroutine */ int zzekllec_(integer *handle, integer *segdsc, integer *
...
/*                    equal to CKEY.  If the column contains elements */
/*                    equal to CKEY, PRVLOC is the index of the last */
/*                    such element. */

/*                    If all elements of the column are greater than */
/*                    CKEY, PRVLOC is set to zero. */
...
/*     Get the record pointers BEGPTR and ENDPTR to the least and */
...
/*        are less than or equal to the input value. */
	...
	*prvloc = 0;
	*prvptr = 0;
    } else if (zzekscmp_(&c__4, handle, segdsc, coldsc, &endptr, &c__1, &c__1,
