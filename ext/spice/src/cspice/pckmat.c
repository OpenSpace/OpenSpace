...
/* $Procedure PCKMAT ( Get transformation matrix at time from PCK file ) */
...
/*      REF      is the integer code for the inertial reference frame of */
/*               the state transformation matrix TSIPM. (See the routine */
/*               CHGIRF for a full list of reference frame names.) */

/*      TSIPM    is a 6x6 transformation matrix.  It is used to transform */
/*               states from inertial coordinates to body fixed (also */
/*               called equator and prime meridian --- PM) coordinates. */
...
/*               intersection of the equator and prime meridian. The Z */
/*               axis points along the spin axis and points towards the */
/*               same side of the invariable plane of the solar system as */
/*               does earth's north pole. */

/*               NOTE: The inverse of TSIPM is NOT its transpose. The */
/*                     matrix, TSIPM, has a structure as shown below: */
...
/*               value .TRUE., otherwise it will have the value .FALSE.. */
...
/*      If the size of the type 03 PCK record to be  retrieved is too */
/*      large to fit into RECORD, the error SPICE(PCKRECTOOLARGE) will be */
/*      signalled. */
...
/*      If a binary PCK file record can be found for the time and */
...
/*           first constant value in the generic segment. */
		...
		*found = FALSE_;
		...
		setmsg_("Storage for # double precision numbers is needed fo"
			"r an PCK data record and only # locations were avail"
			"able. Update the parameter MAXREC in the subroutine "
			"PCKMAT and notify the NAIF group of this problem.", (
			ftnlen)204);
