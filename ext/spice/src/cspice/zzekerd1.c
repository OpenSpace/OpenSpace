/* zzekerd1.f -- translated by f2c (version 19980913).
...
/*     for a specifed d.p. EK column having a type 1 index, using */
/*     dictionary ordering on d.p. data values and record pointers. */
...
/*     NULL       I   Null flag. */
/*     PRVIDX     O   Ordinal position of predecessor of DKEY. */
/*     PRVPTR     O   Pointer to record containing predecessor of DKEY. */
...
/*     DKEY, */
/*     RECPTR         are, respectively, a d.p. key and a pointer to */
...
/*                    the pair (DKEY, RECPTR). */

/*     NULL           is a logical flag indicating whether the input */
/*                    key is null.  When NULL is .TRUE., DKEY is */
...
/*     3)  If the tree is empty, PRVLOC and PRVPTR are set to zero. */
...
/*     to a specified d.p. key, within a specified segment and */
...
/*        input value. */
	...
	*prvptr = 0;
	return 0;
    } else if (zzekscmp_(&c__1, handle, segdsc, coldsc, &begptr, &c__1, &c__2,
	     " ", dkey, &c__0, null, (ftnlen)1) && *recptr < begptr) {
...
/*     Compare the input value to the greatest value in the column. */
    ...
    if (zzekscmp_(&c__5, handle, segdsc, coldsc, &endptr, &c__1, &c__2, " ", 
	    dkey, &c__0, null, (ftnlen)1)) {
...
/*        input value. */
	...
	*prvidx = tsize;
	...
	return 0;
    } else if (zzekscmp_(&c__1, handle, segdsc, coldsc, &endptr, &c__1, &c__2,
	     " ", dkey, &c__0, null, (ftnlen)1) && endptr <= *recptr) {
...
/*        entry at record MIDPTR. */
	...
	if (zzekscmp_(&c__5, handle, segdsc, coldsc, &midptr, &c__1, &c__2, 
		" ", dkey, &c__0, null, (ftnlen)1)) {
