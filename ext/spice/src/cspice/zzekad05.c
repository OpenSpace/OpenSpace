/* zzekad05.f -- translated by f2c (version 19980913).
	...
	isnull)
{
    ...
    extern /* Subroutine */ int dasudi_(integer *, integer *, integer *, 
	    integer *);
    integer remain, colidx, datptr, nlinks, nwrite, prvbas, ptrloc;
    logical fstpag;
...
/*     The entries of class 5 columns are arrays of d.p. values. */
...
/*              current entry is written. */
		...
		if (fstpag) {
		    segdsc[19] = lastw + 1 + nwrite;
		    fstpag = FALSE_;
		} else {
		    segdsc[19] = lastw + nwrite;
		}
	    } else {

/*              Allocate a d.p. data page.  If this is not the first data */
