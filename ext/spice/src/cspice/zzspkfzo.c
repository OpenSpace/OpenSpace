...
/* Subroutine */ int zzspkfzo_(integer *targ, doublereal *et, char *ref, char 
	*abcorr, S_fp obssub, doublereal *starg, doublereal *lt, ftnlen 
	ref_len, ftnlen abcorr_len)
{
    ...
    extern /* Subroutine */ int mxvg_(doublereal *, doublereal *, integer *, 
	    ...
	    *, char *, S_fp, doublereal *, doublereal *, doublereal *, ftnlen,
	     ftnlen), zzvalcor_(char *, logical *, ftnlen), zzcorsxf_(logical 
	    *, doublereal *, doublereal *, doublereal *), vaddg_(doublereal *,
    ...
    extern /* Subroutine */ int vsubg_(doublereal *, doublereal *, integer *, 
	    doublereal *);
    doublereal stemp[6], stobs[6], xform[36]	/* was [6][6] */;
    ...
    doublereal dltctr, corxfm[36]	/* was [6][6] */;
    integer ltsign, obsctr, reqfrm, typeid;
    extern /* Subroutine */ int chkout_(char *, ftnlen), setmsg_(char *, 
	    ftnlen), sigerr_(char *, ftnlen), spkssb_(integer *, doublereal *,
	     char *, doublereal *, ftnlen), spkltc_(integer *, doublereal *, 
...
/*     subroutine provides the state of the observer relative to its */
/*     center of motion. */
...
/*     OBSSUB     I   Observing body state subroutine. */
/*     STARG      O   State of target. */
/*     LT         O   One way light time between observer and target. */

/* $ Detailed_Input */

/*     TARG        is the NAIF ID code for a target body. The target and */
/*                 observer define a state vector having a position */
/*                 component that points from the observer to the */
/*                 target. */
...
/*        ZZSPKFZO begins by computing the geometric position T(ET) of */
...
/*        for ABCORR, ZZSPKFZO computes the position of the target body */
...
/*        selected, ZZSPKFZO computes the position of the target body T */
...
/*        velocity of the target relative to the solar system */
/*        barycenter, expressed in an inertial frame, and C is */
/*        the speed of light. */
...
/*     cycles. If your application requires these additional corrections */
...
/*     observer function easy reader for spk file */
...
/*        with respect to this center. */
	...
	(*obssub)(et, ref, &obsctr, stoctr, ref_len);
	spkgeo_(targ, et, ref, &obsctr, stemp, &ltcent, ref_len);
	if (failed_()) {
	    chkout_("ZZSPKFZO", (ftnlen)8);
	    return 0;
	}
	vsubg_(stemp, stoctr, &c__6, starg);
...
/*        call ZZSPKFAO and return. */
	...
	if (type__ == 1) {
	    zzspkfao_(targ, et, ref, abcorr, (S_fp)obssub, starg, lt, &dlt, 
		    ref_len, abcorr_len);
	    chkout_("ZZSPKFZO", (ftnlen)8);
	    return 0;
	}
...
/*        of the target relative to the observer. */
	...
	zzspkfao_(targ, et, "J2000", abcorr, (S_fp)obssub, state, lt, &dlt, (
		ftnlen)5, abcorr_len);
	if (failed_()) {
	    chkout_("ZZSPKFZO", (ftnlen)8);
	    return 0;
...
/*           for the frame center as seen from the observer. */
	    ...
	    (*obssub)(et, "J2000", &obsctr, stoctr, (ftnlen)5);
	    spkssb_(&obsctr, et, "J2000", stemp, (ftnlen)5);
	    if (failed_()) {
		chkout_("ZZSPKFZO", (ftnlen)8);
		return 0;
	    }
	    vaddg_(stemp, stoctr, &c__6, stobs);
