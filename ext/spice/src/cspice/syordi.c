/* syordi.f -- translated by f2c (version 19980913).
...
/* Subroutine */ int syordi_(char *name__, char *tabsym, integer *tabptr, 
	integer *tabval, ftnlen name_len, ftnlen tabsym_len)
{
    ...
    extern integer sumai_(integer *, integer *), bsrchc_(char *, integer *, 
	    char *, ftnlen, ftnlen);
    integer locval;
    extern /* Subroutine */ int shelli_(integer *, integer *), chkout_(char *,
	     ftnlen);
    integer locsym;
...
/*     If so, sort the components in place. */
	...
	n = tabptr[locsym + 5];
	shelli_(&tabptr[locsym + 5], &tabval[locval + 5]);
    }
    chkout_("SYORDI", (ftnlen)6);
    return 0;
} /* syordi_ */
